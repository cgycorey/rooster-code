...
def render_help(console: Console) -> None:
    ...
    table.add_row("/goal show", "Show the current active goal")
    table.add_row("/goal check", "Ask the agent to assess if the goal is met")
    ...
    async for event in events:
        ...
        if event.type.value == "tool_result":
            ...
            if event.tool_name == "TodoWrite" and event.result_content:
                render_text_panel(console, "Todo", event.result_content, "blue")
                continue
